...
# Should this also be reduced to catches only in federal waters?
RecCatch.fed1 = RecCatch %>% filter(NEW_MODEN==Fish.Mode,fed_closed==1)%>% 
...
#Pivot discard data by wave and year to get open and closed season discards
RecCatch.fed0 = RecCatch %>% filter(NEW_MODEN==Fish.Mode,fed_closed==0)%>% 
  group_by(YEAR,Gulf,WAVE)%>%summarise(sum(B2))
...
#Pivot discard data by wave and year to get open and closed season discards
RecCatch.fed2 = RecCatch %>% filter(NEW_MODEN==Fish.Mode,fed_closed==2)%>% 
  group_by(YEAR,Gulf,WAVE)%>%summarise(sum(B2))
...
write.csv(Discards,paste0(OutputDir,"\\Fed",Fish.Mode,"_Discards.csv"),row.names = FALSE)
